// vatable_expenses_script.js - JavaScript for Vatable Expenses page
...
function saveCompanyInfo() {
    ...
    localStorage.setItem('vatableExpensesCompanyInfo', JSON.stringify(companyInfo));
}

// Load company info from localStorage
function loadCompanyInfo() {
    const savedInfo = localStorage.getItem('vatableExpensesCompanyInfo');
    if (savedInfo) {
...
function clearCompanyInfo() {
    ...
    localStorage.removeItem('vatableExpensesCompanyInfo');
}
...
function addNewRow() {
    ...
    row.innerHTML = `
        ...
        <td>
            <div class="uneditable net-expenses">0.00</div>
        </td>
        <td>
...
function updateCalculations(row) {
    ...
    // Calculate net expenses: Gross Amount / 1.12
    const netExpenses = grossAmount / 1.12;
    row.querySelector('.net-expenses').textContent = netExpenses.toFixed(2);
    
    // Calculate input tax: Net Expenses * 0.12
    const inputTax = netExpenses * 0.12;
    row.querySelector('.input-tax').textContent = inputTax.toFixed(2);
    
    // Get withholding tax rate based on nature of expense
    let withholdingRate = 0;
    if (natureExpense && expenseCategories[natureExpense]) {
    ...
    // Calculate withholding tax: Net Expenses * Withholding Tax Rate
    const withholdingTax = netExpenses * withholdingRate;
...
function updateTotals() {
    ...
    rows.forEach(row => {
        const grossAmount = parseFloat(row.querySelector('.gross-amount').value) || 0;
        const netExpenses = parseFloat(row.querySelector('.net-expenses').textContent) || 0;
    ...
    document.getElementById('totalNetExpenses').textContent = totalNetExpenses.toFixed(2);
...
function updateSummaryRow() {
    ...
    summaryRow.innerHTML = `
        <tr class="summary-row">
            ...
            <td>-</td>
            <td class="uneditable">${totalNetExpenses.toFixed(2)}</td>
...
// Updated exportToExcel function with expense tally
function exportToExcel() {
    ...
    const totalNetExpenses = parseFloat(document.getElementById('totalNetExpenses').textContent) || 0;
        ...
        <head>
            ...
            <xml>
                <x:ExcelWorkbook>
                    <x:ExcelWorksheets>
                        <x:ExcelWorksheet>
                            <x:Name>Vatable Expenses Report</x:Name>
                            <x:WorksheetOptions>
                                <x:DisplayGridlines/>
                                <x:Print>
                                    <x:ValidPrinterInfo/>
            ...
            <title>Vatable Expenses Report - ${formattedDate}</title>
        </head>
        <body>
            <div class="report-title">VATABLE EXPENSES DATA REPORT</div>
            ...
            <div class="section-title">Expenses Transactions</div>
            ...
            <table class="data-table">
                <thead>
                    <tr>
                        ...
                        <th class="col-net">Net Expenses</th>
    ...
    rows.forEach(row => {
        ...
        // Net Expenses
        const netExpensesDiv = row.querySelector('.net-expenses');
        const netExpensesValue = netExpensesDiv ? parseFloat(netExpensesDiv.textContent) || 0 : 0;
        ...
        htmlContent += `
            <tr class="${rowClass}">
                ...
                <td class="currency numeric">${netExpensesValue.toFixed(2)}</td>
    ...
    htmlContent += `
                ...
                <tfoot>
                    <tr class="summary-row">
                        ...
                        <td>-</td>
                        <td class="currency numeric" style="font-weight: bold;">${totalNetExpenses.toFixed(2)}</td>
            ...
            <div class="tally-section">
                <div class="tally-header">
                    EXPENSE CATEGORY TALLY SUMMARY
                </div>
    ...
    sortedCategories.forEach(category => {
        ...
        let categoryType = '';
        let categoryDisplay = '';
        if (category.includes('SERVICES') || category === 'Rental') {
    ...
    htmlContent += `
            ...
            <div class="totals-section">
                ...
                <table class="totals-table">
                    ...
                    <tr>
                        <td class="total-label">Total Net Expenses:</td>
                        <td class="total-value">₱ ${totalNetExpenses.toFixed(2)}</td>
                    </tr>
                    <tr>
            ...
            <div class="footer">
                ...
                <p>Total Transactions: ${rowNumber - 1} items across ${sortedCategories.length} categories</p>
                <p>Authorized by: ${authorizedEmployee} | Prepared by: Vatable Expenses Data Tracker System</p>
                <p>This is a computer-generated report. No signature is required.</p>
                <p>© ${new Date().getFullYear()} Vatable Expenses Data Tracker - Professional VAT-Compliant Expenses Tracking System</p>
            </div>
        </body>
        </html>
    `;
    ...
    link.href = url;
    const fileName = `Vatable_Expenses_Report_${companyName.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.xls`;
    ...
    rows.forEach(row => {
        ...
        if (grossAmountInput && natureExpenseSelect) {
            ...
            if (natureExpense && expenseCategories[natureExpense]) {
                ...
                // Calculate net expenses for this row
                const netExpenses = grossAmount / 1.12;
                expenseTally[natureExpense].totalNetExpenses += netExpenses;
                
                // Calculate withholding tax for this row
                const withholdingTax = netExpenses * expenseCategories[natureExpense];
...
// Add this function to calculate and update expense tally
    ...
    sortedCategories.forEach(category => {
        ...
        tallyItem.innerHTML = `
            <div class="tally-category">
                <i class="fas ${categoryType === 'services' ? 'fa-cogs' : categoryType === 'goods' ? 'fa-box' : 'fa-building'}"></i>
                ${displayCategory}
            </div>
            ...
            <div style="margin-top: 8px; font-size: 11px; color: #80cbc4;">
                Net: ₱ ${data.totalNetExpenses.toFixed(2)} | W/Tax: ₱ ${data.totalWithholdingTax.toFixed(2)}
            </div>
        `;
    ...
    if (totalRows > 0) {
        ...
        emptyItem.innerHTML = `
            ...
            <div style="color: #80cbc4; font-size: 14px; text-align: center; padding: 10px 0;">
                Add expenses to see category totals here
            </div>
        `;
...
// Update the updateTotals function to also update the tally
function updateTotals() {
    let totalGrossAmount = 0;
    let totalNetExpenses = 0;
    let totalInputTax = 0;
...
// Update event listeners to call updateTotals which now includes updateExpenseTally
// The existing event listeners already call updateTotals, so no changes needed there
...
// Update the clearAllData function to refresh tally
function clearAllData() {
    const tableBody = document.getElementById('tableBody');
    tableBody.innerHTML = '';
    addNewRow();
    updateRowCount();
    updateTotals(); // This will also update the tally
    updateSummaryRow();
}

// Update the deleteRow function to refresh tally
function deleteRow(rowId) {
    const row = document.getElementById(rowId);
    if (row && confirm('Are you sure you want to delete this row?')) {
        // Add fade out effect
...
// Update the addNewRow function to refresh tally
// The event listeners added in addNewRow already call updateTotals which now updates tally