// non_vat_purchases_script.js - JavaScript for Non-Vatable Purchases page
...
function saveCompanyInfo() {
    ...
    localStorage.setItem('nonVatablePurchasesCompanyInfo', JSON.stringify(companyInfo));
}

// Load company info from localStorage
function loadCompanyInfo() {
    const savedInfo = localStorage.getItem('nonVatablePurchasesCompanyInfo');
    if (savedInfo) {
...
function clearCompanyInfo() {
    ...
    localStorage.removeItem('nonVatablePurchasesCompanyInfo');
}
...
function addNewRow() {
    ...
    row.innerHTML = `
        ...
        <td>
            <div class="uneditable net-amount">0.00</div>
        </td>
        <td>
...
function updateCalculations(row) {
    ...
    // Calculate withholding amount: Gross Purchases * Withholding Tax Rate
    const withholdingAmount = grossPurchases * withholdingRate;
    row.querySelector('.withholding-amount').textContent = withholdingAmount.toFixed(2);
    
    // Calculate net amount: Gross Purchases - Withholding Amount
    const netAmount = grossPurchases - withholdingAmount;
...
function updateTotals() {
    ...
    rows.forEach(row => {
        ...
        const netAmount = parseFloat(row.querySelector('.net-amount').textContent) || 0;
    ...
    // Net Amount Payable is already calculated as totalGrossPurchases - totalWithholding
    const netPayable = totalGrossPurchases - totalWithholding;
...
function updateSummaryRow() {
    ...
    summaryRow.innerHTML = `
        <tr class="summary-row">
            ...
            <td class="uneditable">${totalNetAmount.toFixed(2)}</td>
            <td>-</td>
            <td>-</td>
        </tr>
    `;
...
function updateTallySections() {
    ...
    rows.forEach(row => {
        ...
        if (paymentMode === 'Cash') {
            totalCashAmount += grossPurchases;
            cashCount++;
        } else if (paymentMode === 'Charge') {
            totalChargeAmount += grossPurchases;
            chargeCount++;
        }
        ...
        if (withholdingRate === 0.01) { // 1%
            ...
            totalZeroPercent += grossPurchases; // Use gross purchases for 0% rate
            zeroPercentCount++;
        }
    });
...
function exportToExcel() {
    ...
    rows.forEach(row => {
        ...
        if (withholdingRate === 0.01) {
            ...
            totalZeroPercent += grossPurchases;
            zeroPercentCount++;
        }
    });
        ...
        <head>
            ...
            <xml>
                <x:ExcelWorkbook>
                    <x:ExcelWorksheets>
                        <x:ExcelWorksheet>
                            <x:Name>Non-Vatable Purchases Report</x:Name>
                            <x:WorksheetOptions>
                                <x:DisplayGridlines/>
                                <x:Print>
                                    <x:ValidPrinterInfo/>
            ...
            <style>
                * {
                ...
                .col-gross { width: 100px; }
                .col-rate { width: 90px; }
                .col-withholding { width: 100px; }
                .col-net { width: 100px; }
                .col-remarks { width: 120px; }
            </style>
            <title>Non-Vatable Purchases Report - ${formattedDate}</title>
        </head>
        <body>
            <div class="report-title">NON-VATABLE PURCHASES DATA REPORT</div>
            ...
            <div class="section-title">Non-Vatable Purchases Transactions</div>
    ...
    rows.forEach(row => {
        ...
        const netAmountDiv = row.querySelector('.net-amount');
        const netAmountValue = netAmountDiv ? parseFloat(netAmountDiv.textContent) || 0 : 0;
        ...
        htmlContent += `
            <tr class="${rowClass}">
                ...
                <td class="currency numeric">${netAmountValue.toFixed(2)}</td>
    ...
    // Add summary row
    let totalNetAmount = totalGrossPurchases - totalWithholding;
    htmlContent += `
                </tbody>
                <tfoot>
                    <tr class="summary-row">
                        ...
                        <td class="currency numeric" style="font-weight: bold;">${totalNetAmount.toFixed(2)}</td>
                        <td>-</td>
                    </tr>
                </tfoot>
            </table>
            ...
            <div class="footer">
                ...
                <p>Authorized by: ${authorizedEmployee} | Prepared by: Non-Vatable Purchases Data Tracker System</p>
                <p>This is a computer-generated report. No signature is required.</p>
                <p>© ${new Date().getFullYear()} Non-Vatable Purchases Data Tracker - Professional Non-VAT Purchases Tracking System</p>
            </div>
        </body>
        </html>
    `;
    ...
    link.href = url;
    const fileName = `Non_Vatable_Purchases_Report_${companyName.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.xls`;
