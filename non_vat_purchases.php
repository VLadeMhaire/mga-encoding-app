...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $has_info) {
    ...
    $stmt = $conn->prepare("INSERT INTO non_vat_purchases (user_id, info_id, date, invoice_number, invoice_type, mode_of_payment, particulars, address, tin_number, vatable_gross_purchases, withholding_tax_rate, remarks) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("iisssssssdss", 
        $user_id, $info_id, $date, $invoice_number, $invoice_type, $mode_of_payment, 
        $particulars, $address, $tin_number, $vatable_gross_purchases, 
        $withholding_tax_rate, $remarks
    );
    
    if ($stmt->execute()) {
        $success = "Non-VAT Purchases record saved successfully!";
    } else {
...
<head>
    ...
    <title>Non-VAT Purchases - MGA&A Encoding App</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <header>
            <h1>Non-VAT Purchases</h1>
            <p>Enter Non-VAT purchase transactions with complete details</p>
        </header>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <div class="amount-input-group">
                                <label for="vatable_gross_purchases">Non-Vatable Gross Purchases</label>
    ...
    <script>
        ...
        function validateForm() {
            ...
            // Validate vatable gross purchases if provided
            if (vatableGrossPurchases.value) {
                ...
                if (purchasesValue && isNaN(parseFloat(purchasesValue))) {
                    alert('Non-Vatable Gross Purchases must be a valid number');
                ...
                if (purchasesValue && parseFloat(purchasesValue) < 0) {
                    alert('Non-Vatable Gross Purchases cannot be negative');
                    vatableGrossPurchases.focus();
                    return false;
                }
            }
