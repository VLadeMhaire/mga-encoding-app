...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $has_info) {
    ...
    $stmt = $conn->prepare("INSERT INTO taxes_licenses (user_id, info_id, date, reference_number, tax_type, mode_of_payment, government_agency, amount, remarks) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("iisssssds", 
        $user_id, $info_id, $date, $reference_number, $tax_type,
        $mode_of_payment, $government_agency, $amount, $remarks
    );
    
    if ($stmt->execute()) {
        $success = "Taxes & Licenses record saved successfully!";
    } else {
...
<head>
    ...
    <title>Taxes & Licenses - MGA&A Encoding App</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <header>
            <h1>Taxes & Licenses</h1>
            <p>Enter taxes and licenses payments with complete details</p>
        </header>
        
        <div class="form-container">
            <?php if (!$has_info): ?>
                <div class="message warning">
                    Please complete the <a href="information.php" style="color: #ffc107;">Company Information</a> section first before entering taxes data.
                </div>
            <?php else: ?>
                ...
                <form method="POST" action="" id="taxForm" onsubmit="return validateForm()">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="reference_number" name="reference_number" placeholder="Enter reference/or number">
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select id="tax_type" name="tax_type" required>
                                ...
                                <option value="DST">DST (Documentary Stamp Tax)</option>
                                <option value="WT">WT (Withholding Tax)</option>
                                <option value="LTO">LTO (Land Transportation Office)</option>
                                <option value="PFP">PFP (Professional Fee Payment)</option>
                                <option value="BPLO">BPLO (Business Permit & Licensing Office)</option>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label for="government_agency" class="required">Government Agency</label>
                            <input type="text" id="government_agency" name="government_agency" placeholder="e.g., BIR, LTO, City Hall" required>
                        </div>
    ...
    <script>
        ...
        function validateForm() {
            ...
            const governmentAgency = document.getElementById('government_agency');
            ...
            if (!governmentAgency.value.trim()) {
                alert('Please enter government agency');
                governmentAgency.focus();
                return false;
            }
            ...
            if (amount.value) {
                ...
                let amountValue = amount.value.replace(/[^0-9.]/g, '');
