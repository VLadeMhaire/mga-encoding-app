// capex_script.js - JavaScript for Capital Expenditures (CAPEX) page
...
function saveCompanyInfo() {
    ...
    localStorage.setItem('capexCompanyInfo', JSON.stringify(companyInfo));
}

// Load company info from localStorage
function loadCompanyInfo() {
    const savedInfo = localStorage.getItem('capexCompanyInfo');
    if (savedInfo) {
...
function clearCompanyInfo() {
    ...
    localStorage.removeItem('capexCompanyInfo');
}
...
function addNewRow() {
    ...
    row.innerHTML = `
        ...
        <td>
            <input type="text" class="editable asset-description" placeholder="Asset description">
        </td>
        <td>
        ...
        <td>
            <input type="number" step="0.01" min="0" class="editable non-vat-gross" placeholder="0.00">
        </td>
        <td>
            <input type="number" step="0.01" min="0" class="editable vatable-gross" placeholder="0.00">
        </td>
        <td>
        ...
        <td>
            <input type="number" step="0.01" min="0" class="editable withholding-tax" placeholder="0.00">
        </td>
        <td>
...
function updateCalculations(row) {
    const nonVatGross = parseFloat(row.querySelector('.non-vat-gross').value) || 0;
    const vatableGross = parseFloat(row.querySelector('.vatable-gross').value) || 0;
    ...
    const totalGross = nonVatGross + vatableGross;
    
    // Calculate Gross Amount minus Withholding Tax
    const grossMinusWithholding = totalGross - withholdingTax;
...
function updateTotals() {
    let totalNonVatPurchases = 0;
    let totalVatablePurchases = 0;
    ...
    rows.forEach(row => {
        ...
        totalNonVatPurchases += nonVatGross;
        totalVatablePurchases += vatableGross;
    ...
    document.getElementById('totalNonVatPurchases').textContent = totalNonVatPurchases.toFixed(2);
    document.getElementById('totalVatablePurchases').textContent = totalVatablePurchases.toFixed(2);
...
function updateSummaryRow() {
    ...
    summaryRow.innerHTML = `
        <tr class="summary-row">
            ...
            <td>-</td>
            <td class="uneditable">${totalNonVatPurchases.toFixed(2)}</td>
            <td class="uneditable">${totalVatablePurchases.toFixed(2)}</td>
...
function exportToExcel() {
    ...
    const totalNonVatPurchases = parseFloat(document.getElementById('totalNonVatPurchases').textContent) || 0;
    const totalVatablePurchases = parseFloat(document.getElementById('totalVatablePurchases').textContent) || 0;
    ...
    let htmlContent = `
        ...
        <head>
            ...
            <title>Capital Expenditures (CAPEX) Report - ${formattedDate}</title>
        </head>
        <body>
            <div class="report-title">CAPITAL EXPENDITURES (CAPEX) DATA REPORT</div>
            ...
            <div class="section-title">Capital Expenditures Transactions</div>
            ...
            <table class="data-table">
                <thead>
                    <tr>
                        ...
                        <th class="col-asset">Asset Description</th>
                        <th class="col-address">Address</th>
                        <th class="col-tin">TIN Number</th>
                        <th class="col-amount">Non Vat Gross</th>
                        <th class="col-amount">Vatable Gross</th>
    ...
    rows.forEach(row => {
        ...
        // Asset Description
        const assetDescriptionInput = row.querySelector('.asset-description');
        const assetDescriptionValue = assetDescriptionInput ? assetDescriptionInput.value : '';
        ...
        // Non Vat Gross Purchases
        const nonVatGrossInput = row.querySelector('.non-vat-gross');
        const nonVatGrossValue = nonVatGrossInput ? parseFloat(nonVatGrossInput.value) || 0 : 0;
        
        // Vatable Gross Purchases
        const vatableGrossInput = row.querySelector('.vatable-gross');
        const vatableGrossValue = vatableGrossInput ? parseFloat(vatableGrossInput.value) || 0 : 0;
        ...
        // Withholding Tax
        const withholdingTaxInput = row.querySelector('.withholding-tax');
        const withholdingTaxValue = withholdingTaxInput ? parseFloat(withholdingTaxInput.value) || 0 : 0;
        ...
        htmlContent += `
            <tr class="${rowClass}">
                ...
                <td class="text-left">${assetDescriptionValue}</td>
                <td class="text-left">${addressValue}</td>
                <td class="text-center">${tinValue}</td>
                <td class="currency numeric">${nonVatGrossValue.toFixed(2)}</td>
                <td class="currency numeric">${vatableGrossValue.toFixed(2)}</td>
    ...
    htmlContent += `
                ...
                <tfoot>
                    <tr class="summary-row">
                        ...
                        <td>-</td>
                        <td class="currency numeric" style="font-weight: bold;">${totalNonVatPurchases.toFixed(2)}</td>
                        <td class="currency numeric" style="font-weight: bold;">${totalVatablePurchases.toFixed(2)}</td>
                        ...
                        <td class="currency numeric" style="font-weight: bold;">${(totalNonVatPurchases + totalVatablePurchases - netPayable).toFixed(2)}</td>
            ...
            <div class="totals-section">
                ...
                <table class="totals-table">
                    <tr>
                        <td class="total-label">Total Non-Vatable Gross Purchases:</td>
                        <td class="total-value">₱ ${totalNonVatPurchases.toFixed(2)}</td>
                    </tr>
                    <tr>
                        <td class="total-label">Total Vatable Gross Purchases:</td>
                        <td class="total-value">₱ ${totalVatablePurchases.toFixed(2)}</td>
                    </tr>
                    <tr>
                        <td class="total-label">Total Gross Purchases:</td>
                        <td class="total-value">₱ ${(totalNonVatPurchases + totalVatablePurchases).toFixed(2)}</td>
                    </tr>
                    <tr>
            ...
            <div class="footer">
                ...
                <p>Authorized by: ${authorizedEmployee} | Prepared by: Capital Expenditures (CAPEX) Data Tracker System</p>
                <p>This is a computer-generated report. No signature is required.</p>
                <p>© ${new Date().getFullYear()} Capital Expenditures (CAPEX) Data Tracker - Professional Asset Acquisition Tracking System</p>
            </div>
        </body>
        </html>
    `;
    ...
    link.href = url;
    const fileName = `CAPEX_Report_${companyName.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.xls`;
