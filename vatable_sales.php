...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $has_info) {
    ...
    $net_sales = !empty($vatable_gross_sales) ? $vatable_gross_sales / 1.12 : 0;
    $output_tax = !empty($vatable_gross_sales) ? $vatable_gross_sales - $net_sales : 0;
    
    $stmt = $conn->prepare("INSERT INTO vatable_sales (user_id, info_id, date, invoice_number, invoice_type, mode_of_payment, particulars, address, tin_number, vatable_gross_sales, net_sales, output_tax, gross_sales_type, exempt_sales, withholding_tax_vat, remarks) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("iissssssssddssss", 
        $user_id, $info_id, $date, $invoice_number, $invoice_type, $mode_of_payment, 
        $particulars, $address, $tin_number, $vatable_gross_sales, $net_sales, 
        $output_tax, $gross_sales_type, $exempt_sales, $withholding_tax_vat, $remarks
    );
    
    if ($stmt->execute()) {
        $success = "Vatable Sales record saved successfully!";
    } else {
...
<head>
    ...
    <title>Vatable Sales - MGA&A Encoding App</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <header>
            <h1>Vatable Sales</h1>
            <p>Enter VAT-able sales transactions with complete details</p>
        </header>
                            ...
                            <div class="amount-input-group">
                                ...
                                <div class="input-with-currency">
                                    ...
                                    <input type="text" id="vatable_gross_sales" name="vatable_gross_sales" 
                                           ...
                                           pattern="[0-9]*\.?[0-9]*"
                                           placeholder="0.00"
                                           oninput="formatAmount(this); calculateNetSales();"
                                           required>
                                </div>
                                <div class="input-hint">Enter amount with decimal point (e.g., 1000.50)</div>
                                <div class="calculation-note">
                                    <strong>Note:</strong> Vatable Gross Sales = Net Sales + 12% Output Tax
                                </div>
                            </div>
                    ...
                    <div class="auto-calc-section">
                        ...
                        <div class="calculation-grid">
                            <div class="calculation-item">
                                ...
                                <div class="calculation-value" id="net_sales_display">₱0.00</div>
                                <input type="hidden" id="net_sales" name="net_sales">
                            </div>
                            <div class="calculation-item">
                                <div class="calculation-label">Output Tax (12% of Net Sales)</div>
                                <div class="calculation-value" id="output_tax_display">₱0.00</div>
                                <input type="hidden" id="output_tax" name="output_tax">
                            </div>
                        </div>
                        <div class="calculation-note" style="margin-top: 15px;">
                            <strong>Formula:</strong><br>
                            <span class="formula">Net Sales = Vatable Gross Sales ÷ 1.12</span><br>
                            <span class="formula">Output Tax = Vatable Gross Sales - Net Sales</span>
                        </div>
                    </div>
    ...
    <script>
        ...
        function formatAmount(input) {
            ...
            // Trigger calculation if this is the vatable gross sales field
            if (input.id === 'vatable_gross_sales') {
                calculateNetSales();
            }
        }
        
        function calculateNetSales() {
            const vatableGrossSalesInput = document.getElementById('vatable_gross_sales');
            ...
            let vatableGrossSales = vatableGrossSalesInput.value.replace(/[^0-9.]/g, '');
            
            if (vatableGrossSales && !isNaN(parseFloat(vatableGrossSales)) && parseFloat(vatableGrossSales) > 0) {
                const grossSales = parseFloat(vatableGrossSales);
                
                // Calculate Net Sales: Vatable Gross Sales ÷ 1.12
                const netSales = grossSales / 1.12;
                
                // Calculate Output Tax: Vatable Gross Sales - Net Sales (or Net Sales * 0.12)
                const outputTax = grossSales - netSales;
                ...
                console.log('Vatable Gross Sales:', grossSales);
                console.log('Net Sales:', netSales);
                console.log('Output Tax:', outputTax);
                console.log('Verification (Net Sales + Output Tax):', (netSales + outputTax).toFixed(2));
            } else {
                // Reset values if input is empty or invalid
                netSalesDisplay.textContent = '₱0.00';
                outputTaxDisplay.textContent = '₱0.00';
                netSalesHidden.value = '0.00';
                outputTaxHidden.value = '0.00';
            }
        }
        ...
        function validateForm() {
            ...
            // Validate vatable gross sales
            if (!vatableGrossSales.value) {
                alert('Vatable Gross Sales is required');
            ...
            if (salesValue && parseFloat(salesValue) <= 0) {
                alert('Vatable Gross Sales must be greater than 0');
                vatableGrossSales.focus();
                return false;
            }
            
            // Check if sales type is selected
            if (salesValue && parseFloat(salesValue) > 0) {
